...
import { getMines } from '../../helpers/apiFetcher'
...
const Grid = ({ level = 'beginner' }) => {
  ...
  const hasWon = useCallback(() => {
    const numberOfCellsOpen = [...cells.values()].reduce((prev, curr) => {
      if (curr.isOpen || curr.content === MINE) {
        return prev++
      }
    ...
    return numberOfCellsOpen === cells.size
  }, [cells])
  ...
  const dialogMessage = useMemo(() => {
    ...
    if (status === 'won') {
      result = {
        open: true,
        message: `🎆 You won the game in ${durationRef.current.duration}`,
      }
    }
    ...
    if (status === 'lost') {
      result = {
        open: true,
        message: `😭 You lost the game in ${durationRef.current.duration}`,
      }
    }
  ...
  const handleCellOpen = (x, y) => {
    ...
    if (cell.content === MINE) {
      setStatus('lost')
      durationRef.current.duration = formatDuration(
        (new Date() - durationRef.current.startedAt) / 1000
      )
      return
    }
  ...
  const renderedCells = [...cells.entries()].map(([cellKey, celValue]) => (
    <GridCell key={cellKey} onOpen={handleCellOpen} {...celValue} />
  ))
    ...
    getMines({
      ...
      setCells((cells) => {
        ...
        for (const [key, value] of cells) {
          const { x, y } = value
          if (newCells.get(key).content === MINE) continue
          let numberOfMinesAround = 0
          for (let i = x - 1; i <= x + 1; i++) {
            for (let j = y - 1; j <= y + 1; j++) {
              const cellKey = `${i}-${j}`
              if (newCells.has(cellKey)) {
                if (newCells.get(cellKey).content === MINE) {
                  numberOfMinesAround++
                }
              }
            }
